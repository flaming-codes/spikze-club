...
worker.addEventListener("install", (event) => {
  event.waitUntil(
    caches
      .open(FILES)
      .then(async (cache) => {
        await cache.addAll(to_cache).catch((error) => console.error("addAll", error));
        await cache
          .addAll(generateSitemapFromBuild(build))
          .catch((error) => console.error("generateSitemapFromBuild", error));
      })
      .then(() => {
        worker.skipWaiting();
      })
